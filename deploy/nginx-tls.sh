...
verify_cert_key_match() {
  ...
  cert_mod="$(openssl x509 -noout -modulus -in "$cert" 2>/dev/null | openssl md5 -r 2>/dev/null | awk '{print $1}')"
  if openssl rsa -in "$key" -noout >/dev/null 2>&1; then
    key_mod="$(openssl rsa -noout -modulus -in "$key" 2>/dev/null | openssl md5 -r 2>/dev/null | awk '{print $1}')"
  else
    key_mod="$(openssl ec -noout -modulus -in "$key" 2>/dev/null | openssl md5 -r 2>/dev/null | awk '{print $1}')"
  fi
...
write_nginx_https_site() {
  ...
  local stapling_block=""
  if [[ "$ssl_kind" == "custom" ]]; then
    stapling_block="
    ssl_stapling on;
...
server {
    ...
    ssl_ecdh_curve auto;${stapling_block}
