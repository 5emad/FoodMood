...
async function refreshReportsAccess() {
  const data = await api('/api/admin/reports/access');
  if (data.success && data.data) {
    reportsAccess = data.data;
  }
  applyReportsAccessUi();
  return reportsAccess;
}

function applyReportsAccessUi() {
  const allowed = Boolean(reportsAccess.allowed);
  const banner = document.getElementById('reportsGateBanner');
  ...
  if (gateText && reportsAccess.message) gateText.textContent = reportsAccess.message;
  if (badge) {
    if (!allowed && reportsAccess.pendingCount > 0) {
      badge.style.display = '';
      badge.textContent = reportsAccess.pendingCount.toLocaleString('fa-IR');
    } else {
...
function goToOrdersForConfirm() {
  activateSidebarTab('orders');
  loadOrders();
  notify(reportsAccess.message || 'ابتدا همه سفارش‌ها را تایید کنید.', 'warning', 'گزارش‌ها');
}
...
async function downloadPdf() {
  if (!reportsAccess.allowed) {
    notify(reportsAccess.message || 'ابتدا همه سفارش‌ها را تایید کنید.', 'warning', 'PDF');
    goToOrdersForConfirm();
    return;
  }
...
async function confirmAllOrders(btn) {
  if (!(await confirmAction({
    title: 'تایید همه سفارش‌های در انتظار؟',
    text: 'تمام سفارش‌هایی که هنوز تایید نشده‌اند یکجا تایید می‌شوند. بعد از تایید، دسترسی به گزارش‌ها باز می‌شود.',
  ...
  btn.disabled = false;
  btn.innerHTML = '<i class="fas fa-check-double"></i> تایید همه سفارش‌های در انتظار';
  if (data.success) {
    notify(data.message || `${data.count} سفارش تایید شد.`);
    await refreshReportsAccess();
    loadOrders();
    if (reportsAccess.allowed) {
      notify('اکنون می‌توانید به بخش گزارش‌ها دسترسی داشته باشید.', 'success', 'گزارش‌ها');
    }
  } else {
